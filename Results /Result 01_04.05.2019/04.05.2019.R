...
m_Assets = 10
linkProbability = 0.4
linkProbability_test_extremumStep = 0.4
linkProbability_test_step = 0.1

# Generating the balance Sheet 
asset_0 = 80
cash_0 = 20
leverage = 10
leverage_test_extremumStep = 5
leverage_test_step = 1
...
meanLiquidity = 0.3
liquidity_step = 0.1
max_liquidityStep = 0.1
min_liqudiityStep = 0.1
liquidity_step_sequence_test = 0.2
...
for (method_selection in c(1, 2, 3)) {
  f = 0
  leverage_test_index = seq(leverage - leverage_test_extremumStep, leverage + leverage_test_extremumStep, by = leverage_test_step)
...
# for (method_selection in c(1, 2, 3)) {
#   f = 0
#   linkProbability_test_index = seq(linkProbability - linkProbability_test_extremumStep, linkProbability + linkProbability_test_extremumStep, linkProbability_test_step)
...
# for (method_selection in c(1, 2, 3)) {
#   f = 0
#   LiquidityHeterogenity_test_index = seq(min_liqudityStep, max_liquidityStep, liquidity_step)
...
x = 1:length(leverage_test_index)
...
axis(side = 1, at= seq(1,length(leverage_test_index),1), leverage_test_index)
legend(x = 8.0, y = 0.3, legend=c("Pro-rata", "Bank Holding Optim", "System Optim"),
...
lines(a3, col="green", lty=2)
axis(side = 1, at= seq(1,length(leverage_test_index),1), leverage_test_index)
legend(x = 8.0, y = -500, legend=c("Pro-rata", "Bank Holding Optim", "System Optim"),
       col=c("blue", "red", "green"), lty=1:2, cex=0.8)
# Note: do box graphs for each of the methods !!
